...
CLASSES = ["cat", "dog", "elephant", "lion", "monkey", "rabbit", "horse", "crocodile", "whale", "penguin"]
"""The classes to train the model on. Only the first 10 classes are used"""
...
def main() -> int:
    ...
    for i in range(ITERS):
        beta = i // (ITERS // 10) # Simulate annealing, by slowly increasing the weight of the KL divergence term
...
def _get_loss(x: torch.Tensor, x_hat: torch.Tensor, x_mean: torch.Tensor, x_logvar: torch.Tensor, beta: float = 1.0) -> torch.Tensor:
    ...
    """
    return F.mse_loss(x_hat, x) + beta * (-0.5 * torch.sum(1 + x_logvar - x_mean.pow(2) - x_logvar.exp()))
